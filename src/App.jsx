...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#060E0C]/80 backdrop-blur-md border-b border-[#00D4B8]/10">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
        <a href="#" className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-md bg-[#00D4B8] flex items-center justify-center">
            <Icon name="zap" className="w-4 h-4 text-[#060E0C]" />
          </div>
          <span className="text-base font-semibold text-white tracking-tight">Pulso</span>
        </a>
        <div className="hidden md:flex items-center gap-7">
          {links.map(([label, href]) => (
            <a key={href} href={href} className="text-sm text-[#6B8A85] hover:text-white transition-colors font-medium">{label}</a>
          ))}
          <a href="#/wiki" className="text-sm text-[#00D4B8] hover:text-[#33DEC6] transition-colors font-medium">Wiki</a>
        </div>
        <div className="hidden md:flex items-center gap-3">
          <a href="#como-funciona" className="text-sm text-[#6B8A85] hover:text-white transition-colors font-medium px-4 py-2">Saiba mais</a>
          <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer"
             className="inline-flex items-center gap-1.5 bg-white hover:bg-gray-100 text-[#060E0C] text-sm font-semibold px-5 py-2 rounded-full transition-colors">
            Falar agora
          </a>
        </div>
        <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-[#6B8A85]">
      ...
      {open && (
        <div className="md:hidden bg-[#060E0C]/98 backdrop-blur-md border-t border-[#00D4B8]/10 px-6 py-5 space-y-4">
          {links.map(([label, href]) => (
            <a key={href} href={href} onClick={() => setOpen(false)} className="block text-sm text-[#6B8A85] hover:text-white">{label}</a>
          ))}
          <a href="#/wiki" onClick={() => setOpen(false)} className="block text-sm text-[#00D4B8]">Wiki de Serviços</a>
          <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer"
             className="block text-center bg-white text-[#060E0C] text-sm font-semibold py-2.5 rounded-full">Falar agora</a>
        </div>
      )}
    </nav>
  )
  ...
  return (
    <section className="min-h-screen flex items-center pt-16 px-6 sm:px-8">
      <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-[1fr_400px] gap-16 items-center py-20">
        ...
        {/* ── Left ── */}
        <div>
          <div className="inline-flex items-center gap-2.5 border border-[#00D4B8]/20 bg-[#00D4B8]/5 rounded-full px-4 py-1.5 mb-8">
            <span className="w-1.5 h-1.5 rounded-full bg-[#00D4B8]" style={{ boxShadow: '0 0 8px #00D4B8' }} />
            <span className="section-label">Diagnóstico gratuito disponível</span>
          </div>
          <h1 className="text-5xl sm:text-6xl lg:text-[72px] font-bold text-white leading-[1.02] tracking-tight mb-6">
            Sua clínica<br />
            merece uma<br />
            <span style={{ color: '#00D4B8', textShadow: '0 0 60px rgba(0,212,184,0.25)' }}>agenda cheia.</span>
          </h1>
          <p className="text-[#6B8A85] text-lg max-w-lg mb-10 leading-relaxed">
          ...
          <div className="flex flex-wrap gap-3 mb-12">
            ...
            <a href="#como-funciona"
               className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full text-sm font-semibold text-[#6B8A85] border border-white/10 hover:border-[#00D4B8]/30 hover:text-white transition-all">
              Ver como funciona
              <Icon name="arrowRight" className="w-4 h-4" />
            </a>
          </div>
          <div className="flex flex-wrap gap-6 text-sm text-[#6B8A85]">
            {[['shield','Conforme CFM/CRM'],['chart','Resultados em 90 dias'],['users','Especialistas em saúde']].map(([icon, label]) => (
              <div key={label} className="flex items-center gap-2">
                <Icon name={icon} className="w-4 h-4 text-[#00D4B8]" />
                {label}
              </div>
            ))}
          </div>
        ...
        <div className="hidden lg:flex flex-col glass-card p-5 gap-4" style={{ borderColor: 'rgba(0,212,184,0.18)' }}>
          <div className="flex items-center justify-between">
            <span className="section-label">PAINEL PULSO</span>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#00D4B8]" style={{ boxShadow: '0 0 6px #00D4B8', animation: 'pulse 2s infinite' }} />
              <span className="text-[11px] text-[#00D4B8] font-medium">Ao vivo</span>
            </div>
          </div>
          {/* Mini ECG */}
          <div className="h-14 bg-[#060E0C]/60 rounded-xl border border-[#00D4B8]/10 flex items-center px-3 overflow-hidden">
            <svg viewBox="0 0 400 40" className="w-full h-9" fill="none">
              <polyline
                ...
                stroke="#00D4B8" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round"
                style={{ filter: 'drop-shadow(0 0 3px rgba(0,212,184,0.7))' }}
              />
            </svg>
          </div>
          {/* 2×2 metrics */}
          <div className="grid grid-cols-2 gap-2.5">
            {[
              ...
              <div key={i} className="bg-[#00D4B8]/5 border border-[#00D4B8]/10 rounded-xl p-3.5">
                <div className="text-[11px] text-[#6B8A85] mb-1.5 font-medium">{m.label}</div>
                <div className="text-white font-bold text-2xl leading-none mb-1">{m.value}</div>
                <div className="text-[11px] text-[#00D4B8]">{m.trend}</div>
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between pt-1 border-t border-[#00D4B8]/10">
            <span className="text-[11px] text-[#6B8A85]">Clínica Derma São Paulo</span>
            <span className="text-[11px] text-[#00D4B8] font-medium">Maio 2025</span>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-14">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            ...
            <p className="text-[#6B8A85] max-w-xs leading-relaxed text-sm">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {points.map((p, i) => (
            <div key={i} className="glass-card p-7 flex flex-col gap-4 hover:border-[#00D4B8]/25 transition-all">
              <div className="flex items-start gap-3">
                ...
                <h3 className="font-semibold text-white text-xl leading-snug">{p.title}</h3>
              </div>
              <p className="text-[#6B8A85] text-sm leading-relaxed">{p.desc}</p>
              <div className="border-t border-white/5 pt-4 space-y-2">
                {p.subs.map((s, j) => (
                  <div key={j} className="flex items-center gap-2 text-xs text-[#6B8A85]">
...
function Services() {
  ...
  const badgeStyle = b => {
    if (b === 'Incluso')      return { bg: 'rgba(0,212,184,0.12)',   color: '#00D4B8',  border: 'rgba(0,212,184,0.3)' }
    ...
    return                           { bg: 'rgba(107,138,133,0.08)', color: '#6B8A85',  border: 'rgba(107,138,133,0.2)' }
  }
  return (
    <section id="servicos" className="py-24 px-6 sm:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 border border-[#00D4B8]/20 bg-[#00D4B8]/5 rounded-full px-4 py-1.5 mb-6">
            <Icon name="zap" className="w-3 h-3 text-[#00D4B8]" />
            <span className="section-label">Serviços</span>
          </div>
          <h2 className="text-4xl sm:text-5xl font-bold text-white mb-4 leading-tight">
            Tudo que sua clínica<br />precisa para crescer
          </h2>
          <p className="text-[#6B8A85] text-lg max-w-xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {services.map((s, i) => {
            ...
            return (
              <div key={i} className="glass-card p-6 flex flex-col gap-4 hover:border-[#00D4B8]/25 transition-all group">
                <div className="flex items-start justify-between">
                  <div className="w-10 h-10 rounded-xl bg-[#00D4B8]/8 border border-[#00D4B8]/15 flex items-center justify-center group-hover:bg-[#00D4B8]/15 transition-colors">
                    <Icon name={s.icon} className="w-5 h-5 text-[#00D4B8]" />
                  </div>
                ...
                <div>
                  <div className="text-[11px] text-[#6B8A85] font-medium mb-1">{s.sub}</div>
                  <h3 className="font-semibold text-white text-base leading-snug">{s.name}</h3>
                </div>
                <p className="text-[#6B8A85] text-sm leading-relaxed flex-1">{s.desc}</p>
                <div className="border-t border-white/5 pt-3 flex flex-wrap gap-1.5">
                  {s.tags.map((t, j) => (
                    <span key={j} className="text-[11px] text-[#6B8A85] bg-white/4 border border-white/8 px-2 py-0.5 rounded-full">{t}</span>
                  ))}
                </div>
              </div>
            )
    ...
    <section id="como-funciona" className="py-24 px-6 sm:px-8">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-start">
        ...
        <div>
          ...
          <p className="text-[#6B8A85] text-base leading-relaxed mb-12 max-w-md">
            ...
            {steps.map((s, i) => (
              <div key={i} className="flex gap-5 group">
                <div className="flex flex-col items-center">
                  <div className="w-9 h-9 rounded-full bg-[#00D4B8]/10 border border-[#00D4B8]/25 flex items-center justify-center shrink-0 group-hover:bg-[#00D4B8]/20 group-hover:border-[#00D4B8]/50 transition-all">
                    <span className="text-[#00D4B8] text-xs font-bold">{s.num}</span>
                  </div>
                  {i < steps.length - 1 && (
                    <div className="w-px flex-1 my-1" style={{ background: 'linear-gradient(to bottom, rgba(0,212,184,0.3), rgba(0,212,184,0.04))' }} />
                  )}
                </div>
                <div className="pb-8">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    <h3 className="font-semibold text-white">{s.title}</h3>
                    <span className="text-[11px] text-[#00D4B8] bg-[#00D4B8]/8 border border-[#00D4B8]/15 px-2 py-0.5 rounded-full">{s.time}</span>
                  </div>
                  <p className="text-[#6B8A85] text-sm leading-relaxed">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Right: progress card */}
        <div className="glass-card p-8" style={{ borderColor: 'rgba(0,212,184,0.15)' }}>
            ...
            {milestones.map((m, i) => (
              <div key={i}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-[#00D4B8]">{m.status}</span>
                </div>
                <div className="h-1.5 rounded-full bg-[#00D4B8]/10 overflow-hidden">
                  <div className="h-full rounded-full transition-all"
                       style={{ width: `${m.pct}%`, background: 'linear-gradient(to right, #00B8A0, #00D4B8)', boxShadow: '0 0 8px rgba(0,212,184,0.4)' }} />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-8 pt-6 border-t border-[#00D4B8]/10 grid grid-cols-2 gap-4">
            <div>
              <div className="text-3xl font-bold text-[#00D4B8]" style={{ textShadow: '0 0 20px rgba(0,212,184,0.3)' }}>+34</div>
              <div className="text-xs text-[#6B8A85] mt-1">Novos pacientes no mês</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-[#00D4B8]" style={{ textShadow: '0 0 20px rgba(0,212,184,0.3)' }}>R$ 28</div>
              <div className="text-xs text-[#6B8A85] mt-1">Custo por paciente captado</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="planos" className="py-24 px-6 sm:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h2 className="text-4xl sm:text-5xl font-bold text-white mb-4 leading-tight">Monte o plano<br />ideal para sua clínica</h2>
          <p className="text-[#6B8A85] text-lg max-w-xl mx-auto">Contrate individualmente ou escolha um pacote. Sem surpresas.</p>
        </div>
        {/* Tabs */}
        <div className="flex justify-center gap-2 mb-10">
          {[['pacotes','Pacotes Completos'],['individual','Serviços Individuais']].map(([val, label]) => (
            <button key={val} onClick={() => setTab(val)}
              className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all ${tab === val ? 'bg-[#00D4B8] text-[#060E0C] shadow-lg shadow-[#00D4B8]/20' : 'glass-card text-[#6B8A85] hover:text-white'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === 'pacotes' && (
          <div className="grid md:grid-cols-3 gap-5">
            {bundles.map((p, i) => (
              <div key={i} className={`relative glass-card p-8 flex flex-col ${p.popular ? 'border-[#00D4B8]/35' : ''}`}
                   style={p.popular ? { boxShadow: '0 0 40px rgba(0,212,184,0.08)' } : {}}>
                {p.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#00D4B8] text-[#060E0C] text-[11px] font-bold px-4 py-1 rounded-full"
                       style={{ boxShadow: '0 0 16px rgba(0,212,184,0.5)' }}>
                    MAIS POPULAR
                  </div>
                )}
                <div className="mb-6">
                  <h3 className="text-2xl font-bold text-white mb-1">Pulso {p.name}</h3>
                  <p className="text-sm text-[#00D4B8] font-medium mb-3">{p.sub}</p>
                  <p className="text-[#6B8A85] text-sm leading-relaxed">{p.desc}</p>
                </div>
                <div className="mb-6 pb-6 border-b border-white/5">
                  <div className="text-[#6B8A85] text-xs mb-1">Investimento mensal</div>
                  <div className="text-white font-bold text-lg">Consulte valores</div>
                  <div className="text-[#6B8A85] text-xs mt-0.5">Contrato mínimo: 6 meses</div>
                </div>
                <ul className="space-y-2.5 flex-1 mb-8">
                  {p.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2.5 text-sm text-[#6B8A85]">
                      <Icon name="check" className="w-4 h-4 text-[#00D4B8] shrink-0 mt-0.5" />
                      {f}
                    </li>
                  ))}
                </ul>
                <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer"
                   className={`block text-center font-semibold py-3.5 rounded-full transition-all text-sm ${p.popular ? 'btn-primary' : 'text-[#00D4B8] border border-[#00D4B8]/25 hover:border-[#00D4B8]/50 hover:bg-[#00D4B8]/5'}`}>
                  Quero esse plano
                </a>
              </div>
            ))}
        ...
        {tab === 'individual' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {individual.map((p, i) => (
              <div key={i} className="glass-card p-6 flex flex-col hover:border-[#00D4B8]/25 transition-all">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-[#00D4B8]/8 border border-[#00D4B8]/15 flex items-center justify-center">
                    <Icon name={p.icon} className="w-5 h-5 text-[#00D4B8]" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-white text-sm leading-tight">{p.name}</h3>
                    <p className="text-xs text-[#00D4B8]">{p.sub}</p>
                  </div>
                </div>
                <p className="text-[#6B8A85] text-sm leading-relaxed mb-4">{p.desc}</p>
                <ul className="space-y-1.5 mb-4 flex-1">
                  {p.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2 text-xs text-[#6B8A85]">
                      <Icon name="check" className="w-3.5 h-3.5 text-[#00D4B8] shrink-0 mt-0.5" />
                      {f}
                    </li>
                  ))}
                </ul>
                <div className="bg-[#00D4B8]/5 border border-[#00D4B8]/10 rounded-lg p-2.5 mb-4">
                  <p className="text-[11px] text-[#6B8A85]">{p.ideal}</p>
                </div>
                <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer"
                   className="block text-center text-sm font-semibold py-2.5 rounded-full text-[#00D4B8] border border-[#00D4B8]/25 hover:border-[#00D4B8]/50 hover:bg-[#00D4B8]/5 transition-all">
                  Saber mais
                </a>
              </div>
            ))}
        ...
        <p className="text-center text-[#6B8A85] text-sm mt-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          {[
            ...
            <div key={i} className="glass-card p-6 flex flex-col justify-between hover:border-[#00D4B8]/25 transition-all">
              <div className="text-4xl sm:text-5xl font-bold text-[#00D4B8] mb-2 leading-none"
                   style={{ textShadow: '0 0 30px rgba(0,212,184,0.3)' }}>
                {m.value}
              </div>
              <div>
                <div className="text-white text-sm font-semibold mb-1">{m.label}</div>
                <div className="text-[#6B8A85] text-xs leading-snug">{m.desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Ad investment guide */}
        <div className="glass-card p-8" style={{ borderColor: 'rgba(0,212,184,0.12)' }}>
          <div className="section-label mb-2">Guia de investimento em anúncios</div>
          <h3 className="text-white text-xl font-bold mb-6">Quanto custa trazer pacientes pelo digital?</h3>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { tier: 'Básico', budget: 'R$ 1–2 mil/mês', leads: '30–60 leads/mês', color: 'rgba(0,212,184,0.8)' },
              { tier: 'Intermediário', budget: 'R$ 2–5 mil/mês', leads: '60–150 leads/mês', color: '#00D4B8' },
              { tier: 'Avançado', budget: 'R$ 5–15 mil/mês', leads: '150–400+ leads/mês', color: '#33DEC6' },
            ].map((t, i) => (
              <div key={i} className="bg-[#00D4B8]/5 border border-[#00D4B8]/10 rounded-xl p-5">
                <div className="section-label mb-2" style={{ color: t.color }}>{t.tier}</div>
                <div className="text-white text-xl font-bold mb-1">{t.budget}</div>
                <div className="text-[#6B8A85] text-sm">{t.leads}</div>
              </div>
            ))}
          </div>
          <p className="text-[#6B8A85] text-xs mt-5">
    ...
    <section className="py-24 px-6 sm:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          {big.map((item, i) => (
            <div key={i} className="glass-card p-8 hover:border-[#00D4B8]/25 transition-all">
              <div className="w-12 h-12 rounded-xl bg-[#00D4B8]/10 border border-[#00D4B8]/20 flex items-center justify-center mb-5">
                <Icon name={item.icon} className="w-6 h-6 text-[#00D4B8]" />
              </div>
              <h3 className="text-xl font-bold text-white mb-3">{item.title}</h3>
              <p className="text-[#6B8A85] text-sm leading-relaxed mb-6">{item.desc}</p>
              <div className="space-y-2.5">
                {item.features.map((f, j) => (
                  <div key={j} className="flex items-center gap-2.5 text-sm text-[#6B8A85]">
                    <Icon name="check" className="w-4 h-4 text-[#00D4B8] shrink-0" />
                    {f}
                  </div>
                ))}
              </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {small.map((item, i) => (
            <div key={i} className="glass-card p-6 hover:border-[#00D4B8]/25 transition-all">
              <div className="w-9 h-9 rounded-lg bg-[#00D4B8]/8 border border-[#00D4B8]/15 flex items-center justify-center mb-4">
                <Icon name={item.icon} className="w-4 h-4 text-[#00D4B8]" />
              </div>
              <h3 className="font-semibold text-white text-sm mb-2">{item.title}</h3>
              <p className="text-[#6B8A85] text-xs leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          {faqs.map((faq, i) => (
            <div key={i} className="glass-card overflow-hidden">
              <button onClick={() => setOpenIdx(openIdx === i ? null : i)}
                      className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-[#00D4B8]/3 transition-colors">
                <span className="font-medium text-white text-sm sm:text-base pr-8 leading-snug">{faq.q}</span>
                <svg className={`w-4 h-4 text-[#00D4B8] shrink-0 transition-transform duration-300 ${openIdx === i ? 'rotate-180' : ''}`}
                     fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path d="m6 9 6 6 6-6"/>
                </svg>
              </button>
              {openIdx === i && (
                <div className="px-6 pb-5 border-t border-[#00D4B8]/8 pt-4">
                  <p className="text-[#6B8A85] text-sm leading-relaxed">{faq.a}</p>
                </div>
              )}
            </div>
          ))}
...
function CTA() {
  return (
    <section className="py-24 px-6 sm:px-8">
      <div className="max-w-5xl mx-auto glass-card overflow-hidden" style={{ borderColor: 'rgba(0,212,184,0.2)', boxShadow: '0 0 80px rgba(0,212,184,0.06)' }}>
        <div className="p-10 sm:p-16">
          ...
          <h2 className="text-5xl sm:text-6xl lg:text-7xl font-bold leading-[1.0] tracking-tight mb-10">
            <span className="text-white">Pronto para lotar</span><br />
            <span style={{ color: 'rgba(0,212,184,0.45)' }}>sua agenda.</span>
          </h2>
          <div className="grid sm:grid-cols-3 gap-8 pt-8 border-t border-[#00D4B8]/10">
            <div>
              <div className="text-xs text-[#6B8A85] uppercase tracking-widest mb-3">WhatsApp</div>
              <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer"
            ...
            <div>
              <div className="text-xs text-[#6B8A85] uppercase tracking-widest mb-3">Diagnóstico</div>
              <div className="text-white font-semibold mb-1">Gratuito</div>
              <div className="text-[#6B8A85] text-sm">Análise completa sem compromisso</div>
            </div>
            <div>
              <div className="text-xs text-[#6B8A85] uppercase tracking-widest mb-3">Resposta</div>
              <div className="text-white font-semibold mb-1">Em até 2h</div>
              <div className="text-[#6B8A85] text-sm">Atendimento de seg–sex, 8h–18h</div>
            </div>
          </div>
        </div>
      </div>
...
function Footer() {
  return (
    <footer className="px-6 sm:px-8 pb-12 pt-8 border-t border-[#00D4B8]/8">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <span className="font-bold text-white text-lg tracking-tight">Pulso</span>
            <span className="text-[#6B8A85] text-sm">por Trion Marketing</span>
          </div>
          <p className="text-[#6B8A85] text-xs">Marketing especializado para clínicas médicas.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 text-sm text-[#6B8A85]">
          <a href="#servicos" className="hover:text-white transition-colors section-label">Serviços</a>
          <a href="#planos" className="hover:text-white transition-colors section-label">Planos</a>
          <a href="#faq" className="hover:text-white transition-colors section-label">FAQ</a>
          <a href="#/wiki" className="hover:text-[#00D4B8] transition-colors section-label text-[#00D4B8]">Wiki</a>
          <span className="text-[#6B8A85] text-xs">© {new Date().getFullYear()} Trion Marketing</span>
        </div>
      </div>
    </footer>
  )
...
function App() {
  ...
  return (
    <div style={{ background: '#060E0C', minHeight: '100vh', position: 'relative', overflowX: 'hidden' }}>
      <EEGCanvas />
